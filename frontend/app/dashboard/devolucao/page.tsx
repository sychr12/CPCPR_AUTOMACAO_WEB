...
import { devApi, municipioApi } from '@/lib/api'
...
export default function DevolucaoPage() {
  ...
  const podeBuscar =
    ...
    memorando.trim().length > 0

  const qPendentes = useQuery({
    queryKey: ['dev-pendentes', munSelecionado, memorando],
    queryFn: () =>
      devApi
        .pendentes(munSelecionado, memorando)
        .then((r) => r.data as Dev[]),
  ...
  const marcar = useMutation({
    mutationFn: () =>
      devApi.marcarEnviado(munSelecionado, memorando),
  ...
  function gerarDocumento() {
    ...
    const dataFmt = new Date().toLocaleDateString(
      'pt-BR',
