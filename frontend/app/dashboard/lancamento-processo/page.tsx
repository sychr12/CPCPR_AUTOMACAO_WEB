...
export default function LancamentoProcessoPage() {
  ...
  const qCpf = useQuery({
    ...
    queryFn: () =>
      inscRenovApi
        .buscar(cpfAtivo)            // ← era buscarPorCpf, não existe
  ...
  const lancar = useMutation({
    ...
    onError: () => toast.error('Erro ao lançar processo'),
  });
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
            ...
            <span style={{
              ...
              {qPendentes.isLoading ? '...' : totalPendente.toLocaleString('pt-BR')}
            </span>
          </div>
                  ...
                  <tbody>
                    {pendentes.map((r: InscRenov) => (
                      <tr key={r.id}>
                        ...
                        <td>
                          <span className="badge badge-insc">{r.unloc}</span>
                        </td>
                        <td style={{ fontFamily: 'JetBrains Mono', fontSize: '11px' }}>
                          {r.memorando}
                        </td>
                        ...
                        <td>
                          <span className={r.descricao === 'INSC' ? 'badge badge-insc' : 'badge badge-renov'}>
                            {r.descricao}
                          </span>
                        </td>
                        <td>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
              <button
                className="btn"
                onClick={() => setPaginaPend(p => Math.max(0, p - 1))}
                disabled={paginaPend === 0}
                style={{ fontSize: 12 }}
              >
                ← Anterior
              </button>
              <button
                className="btn"
                onClick={() => setPaginaPend(p => Math.min(totalPages - 1, p + 1))}
                disabled={paginaPend >= totalPages - 1}
                style={{ fontSize: 12 }}
              >
                Próxima →
              </button>
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              />
              <button className="btn btn-primary" onClick={buscarCpf}>
                🔍
              </button>
            </div>
          </div>
          ...
          {/* RESULTADO CPF */}
          {qCpf.isLoading && <Skeleton />}
          {qCpf.data && qCpf.data.length === 0 && (
            <Vazio texto="Nenhum registro para este CPF" />
          )}
          {qCpf.data && qCpf.data.length > 0 && (
            <div style={{
              background: 'var(--bg-card)',
                ...
                <tbody>
                  {qCpf.data.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.lancou
                          ? <span style={{ color: 'var(--green)' }}>✅ {r.lancou}</span>
...
function Vazio({ texto }: { texto: string }) {
  return (
    <div style={{ padding: '36px', textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{texto}</p>
    </div>
  );
}
